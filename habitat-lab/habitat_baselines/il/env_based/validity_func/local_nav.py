...
from habitat_baselines.il.env_based.validity_func.fmm_plannermine import FMMPlanner
...
class LocalAgent(object):
    def __init__(
        ...
        r = R.from_euler('zyx', [self.curr_rot, 0.0, 0.0])
        self.curr_rot = r.as_quat()
        self.curr_pos = np.array([self.curr_pos[1], 0, -self.curr_pos[0]])
    ...
    def navigate_local(self, rot_limit):
        traversible = (
        ...
        try:
            ...
            ipdb.set_trace()
        planner = FMMPlanner(traversible, 360 // 10, 1, rot_limit)
        ...
        dist = get_l2_distance(start[0], self.goal[0], start[1], self.goal[1])
        if dist < 3:
            terminate = 1
        else:
            terminate = 0
...
def loop_nav_action(
    ...
    nav_length = 0.0
    terminate_local = 0.0
    curr_depth_img = observation["depth"].cpu().numpy()
    local_agent.update_local_map(curr_depth_img)
    local_agent.set_goal(delta_dist, delta_rot)
    action, terminate_local = local_agent.navigate_local(rot_limit)
    return action, terminate_local
...
def loop_nav(
    ...
    for _ in range(max_steps):
        ...
        curr_pos = np.array([curr_pos[1], 0, -curr_pos[0]])
        r = R.from_euler('zyx', [curr_rot, 0.0, 0.0])
        curr_rot = r.as_quat()
...
def map_from_actions(sim, local_agent, start_pos, start_rot, action_list):
    ...
    for action in action_list:
        ...
        curr_position = np.array([curr_position[1], 0, -curr_position[0]])
        r = R.from_euler('zyx', [curr_rotation, 0.0, 0.0])
        curr_rotation = r.as_quat()
