...
class RedNet(nn.Module):
    def __init__(self, num_classes=40, pretrained=False):
        ...
        self.out5_conv_custom = nn.Conv2d(256,
                                          num_classes,
                                          kernel_size=1,
                                          stride=1,
                                          bias=True)
        self.out4_conv_custom = nn.Conv2d(128,
                                          num_classes,
                                          kernel_size=1,
                                          stride=1,
                                          bias=True)
        self.out3_conv_custom = nn.Conv2d(64,
                                          num_classes,
                                          kernel_size=1,
                                          stride=1,
                                          bias=True)
        self.out2_conv_custom = nn.Conv2d(64,
                                          num_classes,
                                          kernel_size=1,
                                          stride=1,
                                          bias=True)
            ...
            if k in state_dict:
                if k.startswith("conv1"):  # the first conv_op
                    ...
                    model_dict[k.replace("conv1", "conv1_d")] = torch.mean(
                        v, 1).data.view_as(state_dict[k.replace(
                            "conv1", "conv1_d")])
...
class RedNetResizeWrapper(nn.Module):
    def __init__(self, device, resize=True, stabilize=False, num_classes=40):
        ...
        #            device=device,
        #        )
        self.semmap_depth_norm = BatchNormalize(mean=[0.213],
                                                std=[0.285],
                                                device=device)
    ...
    def forward(self, rgb, depth, ret_score=False, cutoff=0.0):
        r"""
        Args:
            Raw sensor inputs.
        ...
        if self.training or ret_score:
            scores = self.rednet(rgb, depth)
            return scores
        with torch.no_grad():
            scores = self.rednet(rgb, depth)
            softmax_output = torch.softmax(scores, dim=1)
            max_scores, pred = torch.max(softmax_output, dim=1)
            pred = pred + 1
            mask = torch.where(max_scores < cutoff)
            pred[mask] = 29.0
            pred = pred.float()
        ...
        if self.stabilize:  # downsample tiny
            ...
            pred[
                ~depth_clip] = -1  # 41 is UNK in MP3D, but hab-sim renders with -1
            # pred = F.interpolate(pred.unsqueeze(1), (15, 20), mode='nearest').squeeze(1)
        if self.resize:
            pred = F.interpolate(pred.unsqueeze(1), (og_h, og_w),
                                 mode="nearest")
...
def load_rednet(device, ckpt="", resize=True, stabilize=False, num_classes=40):
    ...
    model = RedNetResizeWrapper(device,
                                resize=resize,
                                stabilize=stabilize,
    ...
    try:
        ...
        prefix = "module."
        state_dict = {(k[len(prefix):] if k[:len(prefix)] == prefix else k): v
        ...
        state_dict.pop("rednet.out2_conv_custom.weight")
        state_dict.pop("rednet.out2_conv_custom.bias")
        state_dict.pop("rednet.out3_conv_custom.weight")
        state_dict.pop("rednet.out3_conv_custom.bias")
        state_dict.pop("rednet.out4_conv_custom.weight")
        state_dict.pop("rednet.out4_conv_custom.bias")
        state_dict.pop("rednet.out5_conv_custom.weight")
        state_dict.pop("rednet.out5_conv_custom.bias")
        # rednet.out2_conv_custom.weight
        # rednet.out2_conv_custom.bias
        print("=> loaded checkpoint '{}' (epoch {})".format(ckpt, "9001"))
