...
class Env:
    ...
    def __init__(
        ...
        self.string_to_label = {"Unknown":255}
        mp3d_map = pd.read_csv('category_mapping.tsv', sep='    ')                              
        goals =['chair', 'bed', 'plant', 'toilet', 'tv_monitor', 'sofa']                                    
#        goals =['bed', 'chair', 'sofa', 'plant', 'toilet', 'tv_monitor']
#        goals =['toilet'] * 6                                    
#        goals =['tv_monitor'] * 6                                    
#        goals =['bed'] * 6                                    
#        goals =['chair'] * 6                                    
        for index, row in mp3d_map.iterrows():                                                              
            if row.mpcat40 in goals:                                                                        
                self.string_to_label[row.raw_category] = goals.index(row.mpcat40)    
            else:
                self.string_to_label[row.raw_category] = 255
        self._config = config
...
#        print("*************START*****************")
#        print(scene.objects[52])
#        print(dir(scene.objects[52]))
#        print(dir(scene.objects[52].semantic_id))
#        print(dir(scene.objects[52].category))
#        print(dir(scene.objects[52].id))
#        print(scene.objects[52].semantic_id)
#        print(scene.objects[52].category)
#        print(scene.objects[52].category.index())
#        print(scene.objects[52].category.name())
#        print(scene.objects[52].id)
#        print("*************END*****************")
        instance_id_to_label_id = {}
        for obj in scene.objects:
            key = int(obj.id.split("_")[-1])
            object_name = obj.category.name()
            if object_name in self.string_to_label:
                instance_id_to_label_id[key] = self.string_to_label[object_name]
            else:
                instance_id_to_label_id[key] = 255
#        instance_id_to_label_id = {int(obj.id.split("_")[-1]): self.string_to_label[obj.category.name()] for obj in scene.objects}
        #instance_id_to_label_id = {int(obj.id.split("_")[-1]): obj.category.index() for obj in scene.objects}
        self.mapping = np.array([ instance_id_to_label_id[i] for i in range(len(instance_id_to_label_id)) ])
        #JUSTIN ADDED THIS HERE TO GET THE SEMANTIC MAPPING
