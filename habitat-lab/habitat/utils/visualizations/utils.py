...
d3_255_colors_rgb = np.zeros((256, 3))
last_values = d3_40_colors_rgb[-1]
d3_255_colors_rgb[:40] = d3_40_colors_rgb
d3_255_colors_rgb[40:] = last_values
...
def paste_overlapping_image(
    ...
    background_patch = background[(
        location[0] - foreground_size[0] // 2 + min_pad[0]):(
            location[0] + (foreground_size[0] - foreground_size[0] // 2)
            - max_pad[0]), (location[1] - foreground_size[1] // 2 + min_pad[1]):(
                location[1] + (foreground_size[1] - foreground_size[1] // 2)
                - max_pad[1]), ]
    foreground = foreground[min_pad[0]:foreground.shape[0] - max_pad[0],
    ...
    if mask is not None:
        mask = mask[min_pad[0]:foreground.shape[0] - max_pad[0],
                    min_pad[1]:foreground.shape[1] - max_pad[1], ]
...
def observations_to_image(observation: Dict, info: Dict) -> np.ndarray:
    ...
    for sensor_name in observation:
        if "rgb" in sensor_name:
            ...
            if not isinstance(sem, np.ndarray):
                sem = sem.cpu().numpy()
            sem = d3_255_colors_rgb[sem].squeeze().astype(np.uint8)
            render_obs_images.append(sem)
